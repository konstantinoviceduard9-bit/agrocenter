...
export function WorkReportsPage() {
  ...
  return (
    <>
      <PageTitle
        title="Отчёты по работам"
        subtitle={
          isManager
            ? 'Журнал выполненных задач, ветобслуживания и передач по коровникам — для смены и руководства.'
            : 'Ваши отмеченные выполнения. Руководитель видит сводку по всем сотрудникам.'
        }
      />
      ...
      <WidgetCard
        title={isManager ? `Записей: ${filtered.length}` : `Мои записи: ${filtered.length}`}
        footer="Демо: отчёты хранятся в браузере на этом устройстве"
      >
        {filtered.length === 0 ? (
          <p className="text-sm text-slate-600">
            Пока нет зафиксированных работ. Отметьте задачу выполненной в{' '}
      ...
      <p className="mt-4 text-xs text-slate-600">
        <Link to="/staff" className="font-medium text-blue-700 hover:underline">
